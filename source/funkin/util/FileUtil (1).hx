...
class FileUtil
{
    ...
    force:Bool = false):Void
  {
    ...
    #elseif mobile
    // Android's SAF save picker only ever hands back a URI for ONE document at a time - there's
    // no "give me a writable folder" API reachable from Haxe without adding native Java code
    ...
    function saveNext():Void
    {
      ...
      saveFile('Choose a location to save ${resource.fileName}...', resource.data, null, function(path:String):Void
      {
        paths.push(path);
        saveNext();
      }, function():Void
      {
        // User cancelled one of the per-file save prompts; stop exporting the rest.
        if (onCancel != null)
        {
          onCancel();
        }
