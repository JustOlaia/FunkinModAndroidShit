...
private class CameraViewportEvents extends haxe.ui.events.Events
{
  ...
  override public function register():Void
  {
    ...
    #if mobile
    // Touchscreens have no middle mouse button and no ALT key, so neither of the desktop pan
    // triggers above is reachable. A plain touch-and-drag (which arrives as a normal primary
    // MOUSE_DOWN/MOUSE_MOVE, same as it does on desktop) should pan the viewport instead.
    if (!hasEvent(MouseEvent.MOUSE_DOWN, _onTouchDown)) registerEvent(MouseEvent.MOUSE_DOWN, _onTouchDown);
    #end
  ...
  override public function unregister():Void
  {
    ...
    #if mobile
    unregisterEvent(MouseEvent.MOUSE_DOWN, _onTouchDown);
    Screen.instance.unregisterEvent(MouseEvent.MOUSE_UP, _onTouchUp);
    #end
  ...
  #if mobile
  function _onTouchDown(e:MouseEvent):Void
  {
    if (_panSource != NONE) return;
    Screen.instance.registerEvent(MouseEvent.MOUSE_UP, _onTouchUp);
    _beginPan(TOUCH);
  }

  function _onTouchUp(e:MouseEvent):Void
  {
    Screen.instance.unregisterEvent(MouseEvent.MOUSE_UP, _onTouchUp);
    if (_panSource != TOUCH) return;
    _endPan();
  }
  #end
